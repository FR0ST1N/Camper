    ...
    suite('POST /api/books with title => create book object/expect book object', function() {
      ...
      test('Test POST /api/books with no title given', function(done) {
        chai.request(server)
          ...
          .end(function(err, res){
            assert.equal(res.status, 200);
            assert.equal(res.text, 'No book title found');
            done();
          });  
      });
    ...
    suite('GET /api/books/[id] => book object with [id]', function(){
      ...
      test('Test GET /api/books/[id] with id not in db',  function(done){
        chai.request(server)
          ...
          .end(function(err, res){
            assert.equal(res.status, 200);
            assert.equal(res.text, 'This Book does not exist');
            done();
          });
      });
      ...
      test('Test POST /api/books/[id] with comment', function(done){
        chai.request(server)
          ...
          .end(function(err, res){
            ...
            assert.property(res.body, 'comments', 'Should contain comments');
            assert.isArray(res.body.comments, 'Comments should be an array');
            assert.include(res.body.comments, 'hello world', 'Comments should be included');
            assert.property(res.body, 'title', 'Should contain title');
            assert.property(res.body, '_id', 'Should contain _id');
            done();
          });
